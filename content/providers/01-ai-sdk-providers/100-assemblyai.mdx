...
- **webSocket** _WebSocketConstructor_
  ...
  header, which the native WebSocket constructor in browsers, Node.js, Deno, and
  Bun cannot send. Pass a header-capable implementation such as the `ws`
  package's `WebSocket`.
...
`endOfTurnConfidence`, `speakerLabel`, `languageCode`, `languageConfidence`, and
the `words` array (word timings in milliseconds). The `finish` metadata includes
the `sessionId`, `sessionDurationSeconds`, and any `speakerRevisions` emitted at
session close when `speakerLabels` is enabled.

Send audio in chunks of 50ms to 1000ms, no faster than real time, and keep
each WebSocket message under 128 KB. The AssemblyAI streaming endpoint
authenticates with a WebSocket header, so use a header-capable WebSocket
implementation such as `ws` in runtimes whose native WebSocket constructor
cannot send headers.

The following top-level provider options also apply to streaming: `prompt`,
`keytermsPrompt`, `languageDetection`, `speakerLabels`, `filterProfanity`,
`redactPii`, `redactPiiPolicies`, `redactPiiSub`, and `domain`. Other top-level
options are pre-recorded only and produce a warning when streaming. The
`streaming` provider options mirror AssemblyAI's streaming connection
parameters:
...
- **includePartialTurns** _boolean_
  ...
  Whether to emit `transcript-partial` parts. Defaults to `true`; the server
  forces `false` when `redactPii` is enabled.
